...
12 场同题对比：ABCD 平均 **85.33**/100，仅提示词平均 **41.42**/100，平均差值 **+43.91**（ABCD − 仅提示词）。

ABCD 多出来的分，主要来自：**多方向强制发散、模式轴贴合、结构可核对、设计候选/运行时未验的诚实封顶**；不是「文笔更华丽」。
...
2. 仅提示词样本代表「无模式机器、无 claim 封顶、常写能开工/能上线」的常见聊天输出；不是攻击某具体厂商模型。
3. 两边都未替代 Unity PlayMode；ABCD 侧明确 runtime-not-run。
