...
路由边界：裸 `ABCD`、`ABCD.Dynamic`、`ABCC.Core` 和 `ABCP.Part` 是 ABC 核心执行身份，应进入 `es-ai-abc-core`。本 Skill 只有在用户明确提出“机制复刻/机制研究/机制转合同”等机制研究动作时才命中；文中出现 ABCD 仅表示机制映射目标，不改变上述路由归属。
