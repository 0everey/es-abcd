...
Static Preflight markers: three generation objectives; visible creative
divergence; generation-before-audit separation.
The exact marker `visible creative divergence` is required for Preflight. Static
Preflight is a contract/readiness check only and may never emit ABCD Accepted.
...
`rejectedBranches`, and `finalDecision`. Static execution may complete the ABCD
semantic branch simulation when the InnovationRun state machine, divergence
tree, counterfactual reviews, capability checkpoints and deterministic replay
receipt are all present. Its acceptance is explicitly scoped to
`ABCD semantic/branch simulation`; it must never claim that an external
provider, Unity, runtime or players executed the stages. Runtime evidence is
an independent additive layer, not a prerequisite for static branch
acceptance.
...
Static Preflight markers: task-scoped InnovationRun; global convergence;
...
Static Preflight markers: **ABCC independent**, **ABCD parity**, **A-to-B**,
