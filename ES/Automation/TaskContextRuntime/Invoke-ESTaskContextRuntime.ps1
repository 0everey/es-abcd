...
switch($Action){
    'Create'{
        ...
        if($null -ne $input.PSObject.Properties['authorityDomain']){$createParams.AuthorityDomain=[string]$input.authorityDomain}
        if($null -ne $input.PSObject.Properties['authorityRiskClass']){$createParams.AuthorityRiskClass=[string]$input.authorityRiskClass}
