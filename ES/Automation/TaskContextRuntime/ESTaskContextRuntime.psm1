...
Import-Module ([IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\AI\ESAIWarningsResultProjection.psm1'))) -ErrorAction Stop
...
function Get-ESTaskContextWarningsSummary([string]$ContextText,[string]$Domain='ai-collaboration') {
    $projectRoot=(Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
    $holder=Add-ESAIWarningsResultProjection -Result ([pscustomobject]@{}) -ContextText $ContextText -Domain $Domain -ConsumerId 'task-context-runtime' -ProjectRoot $projectRoot
...
function Get-ESReceiptHashInput {
    ...
    if($null-ne$Receipt.PSObject.Properties['aiWarnings']){$input.aiWarnings=$Receipt.aiWarnings}
    if($null-ne$Receipt.PSObject.Properties['authorityKernel']){$input.authorityKernel=$Receipt.authorityKernel}
...
function New-ESTaskContextTask {
    ...
    Invoke-ESTaskMutex $TaskId {
        ...
        $warningContext="task context goal=$([string]$goal.goalId) revision=$([string]$goal.goalRevision) domain=$AuthorityDomain risk=$AuthorityRiskClass route=$(@($routePlan.routeKeys)-join ',')"
        $state = [pscustomobject][ordered]@{
            ...
            taskStatus='Active'; contextStatus='Live'; completionDecision='undetermined'; deliveryAcceptance='pending'; aiWarnings=Get-ESTaskContextWarningsSummary -ContextText $warningContext -Domain $AuthorityDomain
