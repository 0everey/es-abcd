...
Invoke-Case 'accepted-transition' {
    ...
    Assert-Equal $s.aiWarnings.authorityRank 2 'AIWarnings authority rank';Assert-Equal $s.aiWarnings.skillAuthorityRank 1 'Skill authority rank';Assert-True (@($s.aiWarnings.mainWarnings).Count -gt 0) 'Task state lacks mainWarnings'
    $receipt=Join-Path $root ('state/'+$s.completionReceipt.path);$receiptObject=[Text.UTF8Encoding]::new($false,$true).GetString([IO.File]::ReadAllBytes($receipt))|ConvertFrom-Json;Assert-True (@($receiptObject.aiWarnings.mainWarnings).Count -gt 0) 'Completion receipt lacks mainWarnings';Assert-Equal $receiptObject.aiWarnings.authorityHash $s.aiWarnings.authorityHash 'AIWarnings authority hash binding'
...
Invoke-Case 'warning-summary-tamper' {
    $root=New-Fixture 'warning-tamper';$s=New-AcceptedState $root;$receipt=Join-Path $root ('state/'+$s.completionReceipt.path)
    $obj=[Text.UTF8Encoding]::new($false,$true).GetString([IO.File]::ReadAllBytes($receipt))|ConvertFrom-Json;$obj.aiWarnings.mainWarnings[0].decision='consume-and-report';[IO.File]::WriteAllText($receipt,($obj|ConvertTo-Json -Depth 20),[Text.UTF8Encoding]::new($false))
    $integrity=Test-ESTaskContextIntegrity -ProjectRoot $root -StoreRoot 'state' -TaskId 'task';Assert-Equal $integrity.status 'failed' 'tampered Warning summary retained integrity'
}
