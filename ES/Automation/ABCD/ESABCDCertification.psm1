...
function Get-ESABCDCertificationWarningsSummary([string]$ContextText,[string]$Domain='ai-collaboration') {
    $projectRoot=(Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
    $holder=Add-ESAIWarningsResultProjection -Result ([pscustomobject]@{}) -ContextText $ContextText -Domain $Domain -ConsumerId 'abcd-certification' -ProjectRoot $projectRoot
...
function New-ESABCDCertificationAssessment {
    ...
    $warningDomain=if($Profile -eq 'ReleaseAcceptance'){'release'}elseif($Profile -eq 'RuntimeAcceptance'){'game-logic'}else{'ai-collaboration'}
    $warningContext="certification candidate subject=$([string]$Subject.kind)/$([string]$Subject.id) profile=$Profile status=evidence-pending"
    $assessment = [ordered]@{
        ...
        aiWarnings = Get-ESABCDCertificationWarningsSummary -ContextText $warningContext -Domain $warningDomain
...
function Test-ESABCDCertificationAssessment {
    ...
    $warningDomain=if($profile -eq 'ReleaseAcceptance'){'release'}elseif($profile -eq 'RuntimeAcceptance'){'game-logic'}else{'ai-collaboration'}
    $warningContext="certification assessment subject=$([string]$Assessment.subject.kind)/$([string]$Assessment.subject.id) profile=$profile status=$status eligible=$eligible issues=$($issues.Count)"
