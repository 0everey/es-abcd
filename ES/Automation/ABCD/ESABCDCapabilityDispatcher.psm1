...
function New-ESABCDModelResponseInvoker {
    ...
    $invoker={param($Context)
        ...
        if($index -lt 0){throw "ABCD_MODEL_RESPONSE_NOT_FOUND:$phase/$mode/$round"};[void]$used.Add($index);$item=$queue[$index];$raw=if($item.PSObject.Properties['outputs']){@($item.outputs)}else{@($item.output)};if(@($raw).Count -eq 0){throw "ABCD_MODEL_RESPONSE_OUTPUT_EMPTY:$phase/$mode/$round"};$mapped=@();foreach($output in $raw){if($output.PSObject.Properties['candidate']){$candidate=$output.candidate;if($null -eq $candidate -or -not $candidate.PSObject.Properties['content']){throw "ABCD_MODEL_RESPONSE_CANDIDATE_INVALID:$phase/$mode/$round"};$mapped+=$candidate}else{$mapped+=$output}};return @($mapped)
    }.GetNewClosure()
    $invoker
}
