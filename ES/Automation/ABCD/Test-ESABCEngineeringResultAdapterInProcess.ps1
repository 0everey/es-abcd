...
try {
 ...
 $result=[pscustomobject]@{status='completed';completionEligible=$true;winnerId='arch-1';contractHash=$hash;evidenceSummary=[pscustomobject]@{eligibleCount=2;implementationEvidenceVerifiedCount=2;dimensionVerificationVerifiedCount=2;repairLoopVerifiedCount=2;repairReceipts=@([pscustomobject]@{architectureId='arch-1';status='completed'},[pscustomobject]@{architectureId='arch-2';status='completed'});independentReplayVerifiedCount=2;providerScoreComparisonStatus='stable'}}
 ...
 try { $result.evidenceSummary.dimensionVerificationVerifiedCount=1; Convert-ESABCEngineeringResultToCandidatePatchPlan -EngineeringResult $result -CandidateEnvelope $envelope -Scenario DesignChange -CurrentHead $head -AuthorizationRef 'in-process-test' -AllowedWriteScopes @($relative)|Out-Null } catch {$incompleteBlocked=$_.Exception.Message -like '*COMPLETION_EVIDENCE*'}
