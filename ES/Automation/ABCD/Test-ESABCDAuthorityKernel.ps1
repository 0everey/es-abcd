...
$preflight = Resolve-ESABCDAuthorityDecision -Mode full-depth -Evidence ([pscustomobject]@{ acceptanceIntent = 'full-acceptance'; requiresCompleteDivergence = $true; requiredCapabilities = @($core); selectedCapabilities = @($core) })
$diverged = Resolve-ESABCDAuthorityDecision -Mode full-depth -Evidence ([pscustomobject]@{ acceptanceIntent = 'full-acceptance'; requiresCompleteDivergence = $true; divergenceCompleted = $true; divergenceReceipt = 'receipt://divergence/1'; requiredCapabilities = @($core); selectedCapabilities = @($core) })
...
$cases = @(
    ...
    [pscustomobject]@{ case = 'full-acceptance-requires-divergence'; status = if ($preflight.status -eq 'blocked' -and $preflight.reasonCode -eq 'ABCD_DIVERGENCE_REQUIRED_FOR_ACCEPTANCE' -and -not $preflight.mechanismsContinue) { 'passed' } else { 'failed' } }
    [pscustomobject]@{ case = 'completed-divergence-opens-gate'; status = if ($diverged.status -eq 'accepted' -and $diverged.divergenceGate.completed) { 'passed' } else { 'failed' } }
)
