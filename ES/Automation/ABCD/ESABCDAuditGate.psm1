...
function New-ESABCDFinalGateReceipt {
    ...
    $kernelEvidence = [pscustomobject][ordered]@{
        ...
        acceptanceIntent = 'full-acceptance'
        requiresCompleteDivergence = $true
        # The kernel receives the same completed-proof facts as this gate.  In
        # earlier revisions the intent flag was set without the corresponding
        # receipt, so every complete static proof was rejected as if divergence
        # had not run.  Keep the receipt scoped to this gate; it is not Unity
        # runtime evidence.
        divergenceCompleted = [bool]$Divergence.complete
        divergenceReceipt = if ([bool]$Divergence.complete) { 'audit-gate-divergence-proof' } else { '' }
    ...
    $warningDomain=if([string]$Plan.scenario -eq 'ReleaseCandidate'){'release'}elseif([string]$Plan.scenario -in @('RuntimeChange','PerformanceCritical')){'game-logic'}else{'ai-collaboration'}
    $warningContext="audit gate scenario=$([string]$Plan.scenario) subject=$([string]$Plan.subjectId) status=$status evidence=$EvidenceLevel issues=$($issues.Count)"
    [pscustomobject][ordered]@{
        ...
        aiWarnings = Get-ESABCDAuditWarningsSummary -ContextText $warningContext -Domain $warningDomain
