...
# Consumer bootstrap: single entry Use-ESABCD.ps1 (Home + Index only; caps load on demand).
$shimDir = Join-Path $TargetRoot 'ES\Automation\ABCD'
$shim = Join-Path $shimDir 'Use-ESABCD.ps1'
$shimSrc = Join-Path $pkg 'ES\Automation\ABCD\Use-ESABCD.ps1'
if (Test-Path -LiteralPath $shimSrc -PathType Leaf) {
    Copy-Item -LiteralPath $shimSrc -Destination $shim -Force
}
elseif (-not (Test-Path -LiteralPath $shim -PathType Leaf)) {
    $shimBody = @(
        '# es-abcd single entry — regenerated fallback'
        '$ErrorActionPreference = ''Stop'''
        '$env:ES_ABCD_GOVERNANCE_MODE = ''portable'''
